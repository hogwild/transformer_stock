...
def slicing(images, n_patches, candle_width):
...
class MyDecoderViTBlock(nn.Module):
    ...
    def forward(self, x, enc_output, src_mask, tgt_mask):
        ...
        out = out + self.cross_attn(out, norm1_enc_output, norm1_enc_output, src_mask)
        out = out + self.mlp(self.norm2(out))
        return out
...
class MyViTAutoEncoder(nn.Module):
    ...
    def forward(self, src_imgs, tgt_imgs):
        ...
        for block in self.decoder_blocks:
            tgt_out = block(tgt_out, src_out, src_mask, tgt_mask)
            
        # Getting the classification token only
        # out = out[:, 0]
        # print(tgt_out.shape)
        return self.mlp(tgt_out) # Map to output dimension, output category distribution
    ...
    def generate_mask(self, src, tgt):
        ...
        tgt_mask = (1 - torch.triu(torch.ones(seq_length, seq_length), diagonal=1)).bool()
...
def main():
    ...
    model = MyViTAutoEncoder((1, 64, 60), (1, 32, 15), n_src_patches=20, n_tgt_patches=5, n_blocks=2, hidden_d=8, n_heads=2, out_d=10).to(device)
    N_EPOCHS = 1000
    LR = 0.005
